...
static json_t * _pb2json_insert_field(const Message& msg, const std::string &key, const std::string &value)
{
	...
	jf = json_string(value.c_str());
	json_object_set_new(root, key.c_str(), jf);
...
static json_t * _pb2jsonex(const Message& msg, const Message& sub_msg)
{
	const Descriptor *d = msg.GetDescriptor();
	const Reflection *ref = msg.GetReflection();
	if (!d || !ref) return 0;
...
static void _json2pbex(Message& msg, Message& sub_msg, json_t *root)
{
	const Descriptor *d = msg.GetDescriptor();
	const Reflection *ref = msg.GetReflection();
	if (!d || !ref) throw j2pb_error("No descriptor or reflection");
	
	int field_index = d->field_count() - 1;
...
static void _json2pb_ignore_field(Message& msg, const char *ignore_field_name, json_t *root)
{
	const Descriptor *d = msg.GetDescriptor();
	const Reflection *ref = msg.GetReflection();
	if (!d || !ref) throw j2pb_error("No descriptor or reflection");
	...
	for (void *i = json_object_iter(root); i; i = json_object_iter_next(root, i))
	{
		const char *name = json_object_iter_key(i);
		json_t *jf = json_object_iter_value(i);
        if (0 == strcmp(name, ignore_field_name)) continue;
...
static void _json2pb_force(Message& msg, json_t *root)
{
	const Descriptor *d = msg.GetDescriptor();
	const Reflection *ref = msg.GetReflection();
	if (!d || !ref) throw j2pb_error("No descriptor or reflection");
...
void json2pb_force(Message &msg, json_t *root)
{
	json_autoptr _auto(root);
...
void json2pb_ex(Message &msg, Message &sub_msg, const char *buf, size_t size)
{
	json_t *root;
	json_error_t error;
...
std::string pb2json_ex(const Message &msg, const Message &sub_msg)
{
	std::string r;

	json_t *root = _pb2jsonex(msg, sub_msg);
	json_dump_callback(root, json_dump_std_string, &r, 0);
	return r;
}
...
void json2pb_ignore_field(google::protobuf::Message& msg, const char *ignore_field_name, const char *buf, size_t size)
{
	...
	_json2pb_ignore_field(msg, ignore_field_name, root);

}

std::string pb2json_insert_field(const google::protobuf::Message &msg, std::string &key, std::string &value)
{
	std::string r;

	json_t *root = _pb2json_insert_field(msg, key, value);
	json_dump_callback(root, json_dump_std_string, &r, 0);
	return r;
}

std::string pb2json_insert_msg(const std::string *keys, const std::string *values, int size,
		                       const std::string &msg_key, const google::protobuf::Message &msg,
							   const std::string &key, const std::string &value)
{
	std::string r;

	json_t *jf = 0, *root = json_object();
	json_autoptr _auto(root);
	
	for (int i = 0; i < size; ++i)
	{
		jf = json_string(values[i].c_str());
		json_object_set_new(root, keys[i].c_str(), jf);
	}
	
	jf = _pb2json_insert_field(msg, key, value);
	json_object_set_new(root, msg_key.c_str(), jf);
	json_t *ret = _auto.release();
	
	json_dump_callback(ret, json_dump_std_string, &r, 0);
	return r;
}
